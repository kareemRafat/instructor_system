...
// Group results by branch_name
$grouped_by_branch = [];
foreach ($result as $row) {
    $branch = $row['branch_name'] ?? 'Not Assigned';
    if (!isset($grouped_by_branch[$branch])) {
        $grouped_by_branch[$branch] = [];
    }
    $grouped_by_branch[$branch][] = $row;
}
?>
...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
        <thead class="text-xs text-gray-700 uppercase bg-gray-200">
            <tr class="text-base">
                <th scope="col" class="w-40 px-6 py-3">Username</th>
                <?php if (hasRole('owner', 'admin')): ?>
                    <th scope="col" class="w-40 px-6 py-3"><span>Salary</span></th>
                <?php endif; ?>
                <th scope="col" class="w-40 px-6 py-3">Email</th>
                <th scope="col" class="w-20 px-6 py-3">Branch</th>
                <th scope="col" class="w-20 px-6 py-3">Status</th>
                <th scope="col" class="w-20 px-6 py-3"><span>Action</span></th>
            </tr>
        </thead>
        <tbody class="font-semibold text-base">
            <?php foreach ($grouped_by_branch as $branch => $instructors): ?>
                <tr class="bg-gray-100 border-b-2  border-orange-200">
                    <td colspan="<?php echo hasRole('owner', 'admin') ? 6 : 5; ?>" class="px-6 py-2 font-bold  text-lg <?= branchIndicator($branch)['textColor'] ?>">
                ...
                <?php foreach ($instructors as $row): ?>
                    <tr class="odd:bg-white even:bg-gray-50 bg-white border-b border-gray-200 hover:bg-gray-50">
                        ...
                        <?php endif; ?>
                        <th scope="row" class="w-40 px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                            <?= !empty($row['email']) ? substr($row['email'], 0, 15) . "..." : '<span class="text-gray-400">Not Assigned</span>' ?>
                        </th>
                        ...
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset <?= $row['is_active'] ? 'text-green-700 ring-green-600/20' : 'text-red-700 ring-red-600/20' ?>">
