...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Fetch deductionDays
    $stmt = $pdo->prepare("
                SELECT id, agent_id, days, reason, created_at, deductions_created_at
                FROM salary_deductions
                WHERE agent_id = :agent_id AND created_at >= :startDate AND created_at < :endDate
                ORDER BY deductions_created_at DESC
            ");
    $stmt->execute([
        ':agent_id' => $agent_id,
    ...
    $deductionDays = $stmt->fetchAll(PDO::FETCH_ASSOC);

    header('Content-Type: application/json');
    echo json_encode(['status' => 'success' , 'deductionDays' => $deductionDays]);
}
