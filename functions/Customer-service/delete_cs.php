...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $resultDelPivot = $DelPivotstmt->execute([$cs_id]);

        // then delete from instructor
        $query = "DELETE FROM instructors WHERE id = ? AND role IN ('cs', 'cs-admin' , 'owner')";
        $stmt = $pdo->prepare($query);
        $result = $stmt->execute([$cs_id]);

        // Commit transaction if both queries succeeded
        $pdo->commit();

        if ($resultDelPivot && $result) {
